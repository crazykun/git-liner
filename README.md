...
- ✅ **子模块友好**: 自动识别嵌套 Git 仓库 / Submodule，从父目录打开也能查到历史
...
### 🗂️ 项目历史视图
- **仓库级提交历史**: 查看整个项目的 Git 提交记录，分页加载
- **修改备注信息**: 右键或标题栏按钮，快速修改最后一次 commit 信息
- **恢复到上次提交**: 一键回退最后一次 commit（`reset --soft HEAD~1`），改动保留在暂存区
- **恢复所有未推送**: 回退所有未推送的本地提交到 upstream，改动保留在暂存区
- **自动切换仓库**: 切换文件时自动检测所属 Git 仓库（含子模块），自动刷新
...
### 嵌套 Git 仓库 / Submodule 兼容
即使打开的是父目录而不是真正的仓库根，也能正常追踪历史：
- 自动通过 `git rev-parse --show-toplevel` 探测当前文件所属的 Git 根目录
- 所有 Git 命令在真实的仓库根目录下执行，文件路径相对于该根目录计算
- 适用场景：将多个独立 Git 仓库放在同一个父目录下打开、Git Submodule 子模块、Monorepo 中的嵌套仓库
...
#### 4. 提示 `文件 "xxx/yyy.go" 不在 Git 仓库中或未被跟踪`，但 `git log` 能查到
**原因**: VSCode 打开的目录是真正 Git 仓库的父目录（常见于把多个独立仓库或 Submodule 放在同一个父目录下打开）。旧版本会以工作区根目录为基准计算路径，导致 Git 找不到文件。
**解决方案**:
- 升级到支持嵌套 Git 仓库 / Submodule 的版本，插件会自动定位到真实的 `git rev-parse --show-toplevel`
- 也可以直接用真实的 Git 仓库根目录作为 VSCode 工作区根来打开
